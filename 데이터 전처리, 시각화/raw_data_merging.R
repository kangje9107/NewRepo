...
climate_total<- outer_join(weather_concat,dust_total,by=c("일시","시도코드"))
...
df_medical_total_1 <- df_medical_total %>% select(성별코드,연령대코드,시도코드,일시,가입자일련번호,입내원일수)
View(df_medical_total_1[1:100,])

#head(df_medical_total)
#View(df_medical_total)


#발생건수 카운트

#View(df_medical_total %>% filter((가입자일련번호 == 9876))) # ------ 가입자번호 진료내역번호2개씩 있는 사람들 조회 

#df_medi_2<- df_medical_temp %>% filter((진료건수==2))
#View(df_medi_2)
df_medical_total_1$일시 <- as.character(df_medical_total_1$일시)
df_medical_total_1$일시 <- as.Date(df_medical_total_1$일시, tryFormats = c("%Y%m%d", "%Y/%m/%d"))

#기상데이터와 진료데이터 연결
analysis_total<- inner_join(df_medical_total_1,climate_total,by=c("일시","시도코드"))
str(analysis_total) # ***************************************************************************************************
#View(climate_total[1:100,])
...
#temp_total<- inner_join(analysis_total,pop,by=c("연령대코드","시도코드"))
View(temp_total)
...
total_bypatient <- df_medical_total_1 %>% select(가입자일련번호, 성별코드, 연령대코드,시도코드, 입내원일수) %>% group_by(가입자일련번호, 성별코드, 연령대코드) %>% summarise(시도코드 = mean(시도코드),입내원일수 = sum(입내원일수)) 
View(total_bypatient[1:100,])
View(df[1:100,])
